...
## Data Types in R
Type is the kind of data (numeric, character, logical), structure is the way the data is organized/collected (vector, matrix, data frame, etc.)
...
vector1[3] # index the third value of the vector (5.3)
...
```{r}
highs <- c(44, 50, 33, 43, 51, 54, 59, 66, 57, 42)
lows <- c(26, 27, 15, 25, 26, 35, 46, 53, 29, 22)
```
...
```{r}
f.to.c <- function(ftemp){
  ctemp <- (ftemp - 32) * (5/9)
  print(ctemp)
}

c.highs <- f.to.c(highs)
c.highs <- round(c.highs, digits = 2)
c.lows <- f.to.c(lows)
c.lows <- round(c.lows, digits = 2)
```

Combine your four vectors into a data frame and add informative column names.

```{r}
forecast <- data.frame(highs, lows, c.highs, c.lows)
names(forecast) <- c("Highs.in.F", "Lows.in.F", "Highs.in.C", "Lows.in.C")
```
...
```{r}
summary(forecast)
sd(forecast$Highs.in.F)
sd(forecast$Lows.in.F)
sd(forecast$Highs.in.C)
sd(forecast$Lows.in.C)
```
...
#format(today, format = "")
>>>>>>> 6217cd69181c101f8cae7815f7325760a177fb4a
...
#Ex2
str_today <- "feb-3-2021"
#Since the format is day-month-year we will use function mdy()
date_obj_today <- mdy(str_today)
date_obj_today
