...
NTL.phys.data.Tue90 <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: Row numbers have nothing to do with the data! It would be much more difficult and risky.
...
NTL.phys.data.ex4 <- NTL.phys.data.ex3 %>% 
  filter(depth == 0) %>%
...
NTL.gather.ex5 <- pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck, names_to = "Location", values_to = "Irradiance")

#Exercise 6: spread temperatureC into more than one column based on the depth
NTL.spread.ex6 <- pivot_wider(NTL.phys.data, names_from = depth, values_from = temperature_C)
```
