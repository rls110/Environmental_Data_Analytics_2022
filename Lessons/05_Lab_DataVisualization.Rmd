...
plot1 <- ggplot(PeterPaul.chem.nutrients[PeterPaul.chem.nutrients$depth == 0.00,], aes(x = daynum, y = temperature_C, color = year4)) + geom_point() + facet_wrap("lakename") + ylab("Surface Temperature in C")
print(plot1)
...
plot3 <- ggplot(EPAair, aes(x = PM2.5, y = Ozone, color = meanLat)) + geom_point(alpha = 0.25) + geom_smooth(method = lm)
print(plot3)
...
ggplot(EPAair) + geom_boxplot(aes(x = as.factor(Year), y = PM2.5))
ggplot(EPAair, aes(x = PM2.5)) + geom_histogram() + facet_wrap("Year")
ggplot(EPAair, aes(x = PM2.5, y = as.factor(Year))) + geom_density_ridges(aes(fill = as.factor(Year))) + ylab("Year") + labs(fill = "Year")
