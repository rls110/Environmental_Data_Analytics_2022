...
> The average ozone level is significantly less than the air quality standard of 50ppm (t=##, p<0.001).
...
```{r}
#Exercise 2:
ggplot(data = EPAair, aes(x = Ozone, fill = as.factor(Year))) + 
  geom_density(alpha = 0.5) + 
  labs(title = "Density Plot: Ozone by year", x = "Ozone AQI Value", 
       y = "Density", fill = "Year") + 
  scale_fill_manual(values = c("blue", "yellow"))

ggplot(data = EPAair, aes(x = as.factor(Year), y = Ozone)) + 
  geom_boxplot() + stat_summary(geom = "text", fun = max, vjust = -1, 
                                size = 4, label = c("a", "b"), 
                                color = "red") + 
  labs(x = "", y = "Ozone AQI Value") + ylim(0,150)
...
lm.1 <- lm(PM2.5 ~ Ozone, data = EPAair)
summary(lm.1)
  # R2 = 13.16%, p < 0.001

#Exercise 4: Build a scatterplot. Add a line and standard error for the linear regression. 
ggplot(data = EPAair, aes(x = Ozone, y = PM2.5)) + geom_point() + geom_smooth(method = lm, col = "blue", se = F)

```

Ozone concentration has a statiscally significant relationship with particulate matter concentration (R2 = ##, F = ##, p < 0.001). Specifically, ozone accounts for approximately 13% of the variance in PM.
...
EPAcor <- cor(EPAair%>% select(meanLat:Ozone) %>% drop_na())
corrplot(EPAcor, method = "ellipse")
corrplot.mixed(EPAcor, upper = "ellipse")
...
lm.2 <- lm(data = EPAair, PM2.5 ~ Ozone + Year + Month + meanLat + meanLng)
summary(lm.2)
...
lm.3 <- lm(data = EPAair, PM2.5 ~ Ozone + Month + meanLat + meanLng)
summary(lm.3)
