...
ggplot(BRA_complete, aes(x = Date, y = Rainfall)) + geom_line() + 
  geom_smooth(method = lm)
ggplot(BRA_complete, aes(x = Date, y = Temperature_C)) + geom_line() + 
  geom_smooth(method = lm)

# Temperature displays a clear positive trend, while there is only a slight positive trend in rainfall
...
BRA.temp.ts <- ts(BRA_complete$Temperature_C, start = c(1901,1), frequency = 12)
BRA.rf.ts <- ts(BRA_complete$Rainfall, start = c(1901,1), frequency = 12)
...
temp_decomp <- stl(BRA.temp.ts, s.window = "periodic")
rf_decomp <- stl(BRA.rf.ts, s.window = "periodic")
...
temp.comps <- as.data.frame(temp_decomp$time.series[,1:3])
rf.comps <- as.data.frame(rf_decomp$time.series[,1:3])

temp.comps <- mutate(temp.comps, 
                     Observed = BRA_complete$Temperature_C,
                     Date = BRA_complete$Date)
rf.comps <- mutate(rf.comps,
                   Observed = BRA_complete$Rainfall,
                   Date = BRA_complete$Date)

# Exercise 10: Visualize how the trend maps onto the data for both series
ggplot(temp.comps) + geom_line(aes(x = Date, y = Observed), size = 0.25) + 
  geom_line(aes(x = Date, y = trend), color = "purple")

ggplot(rf.comps) + geom_line(aes(x = Date, y = Observed), size = 0.25) + 
