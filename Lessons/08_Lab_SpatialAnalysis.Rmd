...
huc8_sf <- read_sf("./Data/Spatial/NCHUC8.shp")

#View the data as a map, colored by the acreage in each
ggplot(huc8_sf) + geom_sf(aes(fill = ACRES)) + 
...
#Create a map
ggplot(huc8_sf) + geom_sf(fill = "orange") + 
  geom_sf(data = huc8_sf_UN, fill = "purple")
...
wind_energy_huc8 <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c("HUC_12" = "factor")) %>%
  ...
  group_by(HUC_8) %>%
  summarize(WindPower = sum(AvgWindEnergy))

#Join to HUC_8 features
huc8_sf_wind <- left_join(huc8_sf, wind_energy_huc8, by = "HUC_8")

#View the outputs
head(huc8_sf_wind)

ggplot(huc8_sf_wind) + geom_sf(aes(fill = WindPower))
```
